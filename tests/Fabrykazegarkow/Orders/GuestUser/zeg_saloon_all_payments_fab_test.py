...
def home_page(page: Page, env):
    ...
    open_page(page, env['URL_FAB'])
    home_page.wait_for_home_page()

    expect(page).to_have_title(PageTitles.FABRYKA_HOME_PAGE_TITLE)
    return home_page
...
def test_buy_product_with_payment_method(checkout_summary_page: CheckoutSummaryPage, payment_method_name, page: Page):
    ...
    # pytest -v --env=prod tests\Fabrykazegarkow\Orders\GuestUser\zeg_saloon_and_all_payments_test.py --headed --alluredir=C:\Users\Dima\Desktop\Testing\Python_Playwright\ZegarowniaProject\allure_results
