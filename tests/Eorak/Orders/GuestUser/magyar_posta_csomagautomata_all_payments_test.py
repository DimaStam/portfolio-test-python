...
from library.testdata.Orders.PaymentMethods.EorakPaymentMethods import EorakPaymentMethods
...
def checkout_summary_page(page: Page, env):
    ...
    open_page(page, env['URL_EOR_PRODUCT'])
    ...
    checkout_shipping_page.select_delivery_method(DeliveryMethods.magyar_posta_csomagautomata)
    checkout_shipping_page.select_magyar_delivery_point()
...
@pytest.mark.parametrize("payment_method_name", EorakPaymentMethods.selectors.keys())
def test_buy_product_with_payment_method(checkout_summary_page: CheckoutSummaryPage, payment_method_name, page: Page):
    payment_methods = EorakPaymentMethods(page)
    payment_methods.select_payment_method(payment_method_name)
    if payment_method_name == EorakPaymentMethods.CARD:
        submit_button_locator = checkout_summary_page.card_submit_button_eorak
        checkout_summary_page.fill_card_fields(submit_button_locator)
    
    checkout_summary_page.add_order_comment()
    checkout_summary_page.select_agreement_checkbox(CheckoutSummaryPage.eorak_agreement_checkbox)
    time.sleep(1)
    ...
    # pytest -v --env=prod tests/Eorak/Orders/GuestUser/magyar_posta_csomagautomata_all_payments_test.py --headed --alluredir=/Users/global/Desktop/Zegarownia/allure_results
